    ...
    <>
      <section id="menu" className="py-24 px-6 max-w-7xl mx-auto border-t border-stone-200 dark:border-zinc-800">
        <div className="flex flex-col md:flex-row items-center justify-between mb-16 text-center md:text-left gap-6">
          ...
          <a href="/api/recipes/download-pdf" target="_blank" rel="noreferrer" className="flex items-center gap-2 bg-orange-500 hover:bg-orange-600 text-white px-6 py-3 rounded-full font-bold transition-all duration-300 shadow-md hover:shadow-lg transform hover:-translate-y-1">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {recipes.filter(r => r.menuType !== 'vegetarian').map((item, index) => {
                ...
                return (
                <div key={item.id} onClick={() => setSelectedRecipe({ ...item, imageUrl })} className="cursor-pointer bg-white dark:bg-zinc-900 border border-stone-100 dark:border-zinc-800 rounded-3xl p-4 flex items-center gap-4 hover:shadow-xl transition-shadow duration-300 group hover:-translate-y-1">
